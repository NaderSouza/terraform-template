...
**Propósito:** Esta conta é dedicada ao gerenciamento centralizado dos recursos de rede, como VPCs, sub-redes, tabelas de rotas e gateways.

**Recursos Gerenciados:**
- **Configuração de Base:** Aplica uma configuração de segurança essencial.
- **Access Analyzer (`access-analyzer.tf`):** Monitora as políticas de acesso para os recursos de rede.
- **Contatos Alternativos (`alternate-contacts.tf`):** Define os contatos da conta.
- **Políticas de Senha do IAM (`iam_account.tf`):** Garante a segurança das credenciais de acesso.
- **Acesso para SSM (`iam_role_ssm.tf`):** Permite o gerenciamento seguro das instâncias de rede.
