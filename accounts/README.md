...
Cada diretório de conta (ex: `audit-services`) segue uma estrutura padrão:

- **`setup-terraform-backend/`**: Contém a configuração do _backend_ remoto do Terraform.
...
- **IAM Access Analyzer, Contatos Alternativos, Criptografia EBS, Políticas de Senha, Acesso para Suporte/SSM, Bloqueio de S3 e Service Quotas:** Estes recursos são configurados de forma idêntica à conta `audit-services` para garantir um ambiente seguro e bem gerenciado.
